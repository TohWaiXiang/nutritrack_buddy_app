...
class LocalStore {
  ...
  Future<Map<String, UserProfile>> _getUsers() async {
    ...
    return decoded.map((key, value) => MapEntry(key, UserProfile.fromMap(Map<String, dynamic>.from(value))));
  }
  ...
  Future<bool> register(UserProfile user) async {
    ...
    users[user.email.trim().toLowerCase()] = user.copyWith(email: user.email.trim().toLowerCase());
  ...
  Future<bool> resetPassword({
    ...
    if (user.securityAnswer.trim().toLowerCase() != answer.trim().toLowerCase()) return false;
  ...
  Future<void> saveProfile(UserProfile profile) async {
    final users = await _getUsers();
    users[profile.email.trim().toLowerCase()] = profile.copyWith(email: profile.email.trim().toLowerCase());
    await _saveUsers(users);
  }
  ...
  Future<Map<String, List<MealEntry>>> getMeals(String email) async {
    ...
    return decoded.map((key, value) => MapEntry(
          key,
          (value as List).map((e) => MealEntry.fromMap(Map<String, dynamic>.from(e))).toList(),
        ));
  }

  Future<void> saveMeals(String email, Map<String, List<MealEntry>> meals) async {
    final prefs = await _prefs;
    final encoded = meals.map((key, value) => MapEntry(key, value.map((e) => e.toMap()).toList()));
  ...
  Future<List<GroceryItem>> getGroceries(String email) async {
    ...
    return decoded.map((e) => GroceryItem.fromMap(Map<String, dynamic>.from(e))).toList();
  }

  Future<void> saveGroceries(String email, List<GroceryItem> items) async {
    final prefs = await _prefs;
    await prefs.setString(_groceryKey(email), jsonEncode(items.map((e) => e.toMap()).toList()));
  }
  ...
  Future<List<ChatMessage>> getChat(String email) async {
    ...
    if (raw == null || raw.isEmpty) {
      return const [
        ChatMessage(
          isUser: false,
          text: 'Hi Buddy! I can help you review meals, calories, protein, carbs, and fat. Ask me anything about your food.',
        ),
      ];
    }
    final decoded = jsonDecode(raw) as List;
    return decoded.map((e) => ChatMessage.fromMap(Map<String, dynamic>.from(e))).toList();
  }

  Future<void> saveChat(String email, List<ChatMessage> chat) async {
    final prefs = await _prefs;
    await prefs.setString(_chatKey(email), jsonEncode(chat.map((e) => e.toMap()).toList()));
  }
  ...
  Future<void> seedDefaultsIfNeeded(String email) async {
    ...
    if (groceries.isEmpty) {
      await saveGroceries(email, [
        const GroceryItem(id: 'g1', category: 'Vegetables', name: 'Broccoli', quantity: '2 heads'),
        const GroceryItem(id: 'g2', category: 'Vegetables', name: 'Carrot', quantity: '500g', purchased: true),
        const GroceryItem(id: 'g3', category: 'Vegetables', name: 'Spinach', quantity: '1 pack'),
        const GroceryItem(id: 'g4', category: 'Protein', name: 'Chicken Breast', quantity: '500g', purchased: true),
        const GroceryItem(id: 'g5', category: 'Protein', name: 'Eggs', quantity: '12 pcs'),
        const GroceryItem(id: 'g6', category: 'Carbohydrates', name: 'Whole Wheat Bread', quantity: '1 loaf', purchased: true),
        const GroceryItem(id: 'g7', category: 'Carbohydrates', name: 'Brown Rice', quantity: '1 kg'),
      ]);
    }
  }
